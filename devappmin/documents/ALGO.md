...
길이가 3이상인 경우, start와 end의 값이 같을 경우(j\_idx와 j\_idx에서 i\_idx만큼 떨어진 상황)와 start + 1(j\_idx + 1)과 end - 1(j\_idx에서 i\_idx만큼 떨어진 곳에서 - 1)을 한 값이 1일 경우(팰린드롬일경우), 1로 값을 변경한다.
